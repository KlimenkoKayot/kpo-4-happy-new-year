...
func NewMessage(messageType, payload string) *Message {
	return &Message{
...
func ReconstructMessage(id, messageType, payload string, status Status, createdAt time.Time, processedAt *time.Time, retryCount int) *Message {
	return &Message{
		id:          MessageID(id),
		messageType: messageType,
		payload:     payload,
